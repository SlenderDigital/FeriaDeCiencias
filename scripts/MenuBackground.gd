extends Control
## NeonBackground — Patchbay world background: animated perspective grid + pulse rings on beat.
## Emits clean `beat` signal for UI sync. Grid re-themes to selected track color.
# forced reparse
...
func _trigger_beat_pulse() -> void:
	...
	beat.emit()  # Clean signal for UI sync
...
func _draw() -> void:
	...
	# 1. Base dark synth gradient (panel ink)
	draw_rect(Rect2(Vector2.ZERO, vp_size), Color(0.08, 0.08, 0.12, 1.0))
	
	# 2. Animated perspective horizon grid (silkscreen lines)
	var horizon_y: float = vp_size.y * 0.52
	var center_x: float = vp_size.x * 0.5
	
	# Horizon glow line
	draw_line(Vector2(0, horizon_y), Vector2(vp_size.x, horizon_y), grid_color * 1.5, 1.5)
	...
	for i in range(num_perspective_lines + 1):
		...
		draw_line(Vector2(top_x, horizon_y), Vector2(bottom_x, vp_size.y), grid_color, 1.0)
	...
	for j in range(num_horizontal_lines):
		...
		draw_line(Vector2(margin, line_y), Vector2(vp_size.x - margin, line_y), line_col, 1.0)
		
	# 3. Concentric pulse rings (emitted on beat)
	var center: Vector2 = Vector2(vp_size.x * 0.5, vp_size.y * 0.4)
	for ring in pulse_rings:
		var c: Color = ring["color"]
		c.a = ring["alpha"]
		draw_arc(center, ring["radius"], 0, TAU, 48, c, 2.0)
		
	# 4. Floating particles (silkscreen specks)
	for p in particles:
